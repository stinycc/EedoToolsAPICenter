...
<b>미래</b>를 예측하기 위한 분석에 <b>시간</b>적 요소가 빠져있어도 괜찮을까요? 당신의 분석에는 과연 <b>시간</b>이 포함되어 있나요?\
...
```
\
<b>1. 사용자가 원하는 기간에 따른 (Ex => 분기별, 월별, 일별, 등등) 키워드의 "관심도"와 그 "추세값"를 반환합니다.</b>
- "관심도"는 스크랩한 포스트의 총 갯수를 100으로 환산했을 때 몇개의 포스트에서 해당 키워드를 사용하고 있는지를 나타내는 지수입니다.
- "추세값"은 "관심도"의 평균을 0 으로 고정시켜놓은 것으로 해당 기간에 평균보다 많은 관심을 받은경우 양의 값(+)을 평균보다 적은 관심을 받은경우 음의 값(-)을 보이게됩니다. 해당 값으로 특정 키워드가 단기간에 집중된 관심을 받았는지, 큰 파동이 없는 일정한 관심을 받고 있는지, 시간적 트렌드에 따른 주기를 갖는지 등의 추세를 평가할 수 있습니다.


<b>2. 키워드와 매출간의 상관관계를 반환하며 추가로 보조지표로 사용할 수 있는 분산값을 제공합니다.</b>
...
\
#<b>인풋으로 업로드해야 하는 데이터 구조는 다음과 같습니다</b>\
\.... 추가중
